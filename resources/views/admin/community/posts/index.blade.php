                ...
                <div class="card">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="card-header">
                            {{ __('community.posts.title') }}
                            <small class="text-secondary">{{ __('community.posts.subtitle') }}</small>
                        </h5>
                        <div class="pe-5">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="community-posts-table" class="table table-striped table-hover">
                                <thead class="table-dark">
                                    <tr>
                                        <th>{{ __('common.no') }}</th>
                                        <th>{{ __('community.posts.fields.title') }}</th>
                                        <th>{{ __('community.posts.fields.author_name') }}</th>
                                        <th>{{ __('community.posts.fields.status') }}</th>
                                        <th>{{ __('community.posts.fields.pinned') }}</th>
                                        <th>{{ __('community.posts.fields.published_at') }}</th>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            $('#community-posts-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: '{{ route('admin.community.posts.index') }}',
                    type: 'GET'
                },
                columns: [{
                        data: 'DT_RowIndex',
                ...
                language: {
                    ...
                    zeroRecords: "{{ __('datatable.posts.zeroRecords') }}",
                    emptyTable: "{{ __('datatable.posts.emptyTable') }}",
                    paginate: {
