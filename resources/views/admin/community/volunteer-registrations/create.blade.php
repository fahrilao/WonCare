...
@section('title', __('community.volunteer_registrations.create_title'))
            ...
            <div class="col-md-10 offset-md-1">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4>{{ __('community.volunteer_registrations.create_title') }}</h4>
                        <a href="{{ route('admin.community.volunteer-registrations.index') }}"
                            class="btn btn-secondary btn-sm">{{ __('common.back_to_list') }}</a>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.community.volunteer-registrations.store') }}" method="POST">
                            @csrf
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select{{ $errors->has('type') ? ' is-invalid' : '' }}"
                                        ...
                                        @php $value = old('type', 'digital'); @endphp
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select{{ $errors->has('status') ? ' is-invalid' : '' }}"
                                        ...
                                        @php $value = old('status', 'new'); @endphp
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control{{ $errors->has('availability') ? ' is-invalid' : '' }}" id="availability"
                                    name="availability" rows="3">{{ old('availability') }}</textarea>
                                @error('availability')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
