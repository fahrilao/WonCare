...
@section('title', __('community.whatsapp_groups.edit_title'))
            ...
            <div class="col-md-8 offset-md-2">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4>{{ __('community.whatsapp_groups.edit_title') }}</h4>
                        <a href="{{ route('admin.community.whatsapp-groups.index') }}"
                            class="btn btn-secondary btn-sm">{{ __('common.back_to_list') }}</a>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.community.whatsapp-groups.update', $group) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control{{ $errors->has('region') ? ' is-invalid' : '' }}"
                                    id="region" name="region" value="{{ old('region', $group->region) }}" required>
                                @error('region')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text"
                                    ...
                                    value="{{ old('whatsapp_link', $group->whatsapp_link) }}" required>
                                @error('whatsapp_link')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" id="description"
                                    name="description" rows="4">{{ old('description', $group->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number"
                                    ...
                                    id="sort_order" name="sort_order" value="{{ old('sort_order', $group->sort_order) }}"
                                    min="0">
                                @error('sort_order')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
