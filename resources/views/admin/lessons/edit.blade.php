            ...
            <div class="col-md-10 offset-md-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.lessons.update', $lesson ?? 1) }}" method="POST"
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control{{ $errors->has('content') ? ' is-invalid' : '' }}" id="content" name="content"
                                    rows="6" placeholder="Enter lesson content">{{ old('content', $lesson->content ?? '') }}</textarea>
                                @error('content')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select{{ $errors->has('type') ? ' is-invalid' : '' }}"
                                            ...
                                            <option value="text"
                                                {{ old('type', $lesson->type ?? '') == 'text' ? 'selected' : '' }}>
                                                {{ __('lessons.type_text') }}
                                            </option>
                                            <option value="quiz"
                                                {{ old('type', $lesson->type ?? '') == 'quiz' ? 'selected' : '' }}>
                                                {{ __('lessons.type_quiz') }}
                                            </option>
                                            <option value="assignment"
                                                {{ old('type', $lesson->type ?? '') == 'assignment' ? 'selected' : '' }}>
                            ...
                            <div id="video-fields" style="display: none;">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select{{ $errors->has('video_source') ? ' is-invalid' : '' }}"
                                        ...
                                        <option value="youtube"
                                            {{ old('video_source', $lesson->video_source ?? '') == 'youtube' ? 'selected' : '' }}>
                                            {{ __('lessons.video_source_youtube') }}
                                        </option>
                                        <option value="upload"
                                            {{ old('video_source', $lesson->video_source ?? '') == 'upload' ? 'selected' : '' }}>
                                ...
                                <div id="upload-field" class="mb-3" style="display: none;">
                                    ...
                                    @if ($lesson->video_file ?? false)
                                        <div id="current-video-info" class="alert alert-info mb-3">
                                            <i class="ti tabler-video me-2"></i>
                                            {{ __('lessons.current_video') }}:
                                            <strong>{{ basename($lesson->video_file) }}</strong>
                                        </div>
                                    @endif

                                    <div id="video-dropzone" class="border border-dashed rounded p-4 text-center"
                                        style="cursor: pointer; background: #f8f9fa;">
                                        <div id="dropzone-content">
                                            <i class="ti tabler-cloud-upload icon-xl text-muted mb-2"></i>
                                            <p class="mb-1">{{ __('lessons.drag_drop_video') }}</p>
                                            <small class="text-muted">{{ __('lessons.video_file_help_edit') }}</small>
                                        </div>
